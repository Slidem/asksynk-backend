...
describe("CalendarEventsController (integration)", () => {
  ...
  it("POST /calendar-events creates an event and persists it to the DB", async () => {
  ...
  it("POST /calendar-events creates a recurring event", async () => {
    const eventId = generateId();
  ...
  it("GET /calendar-events expands recurring event into instances", async () => {
    const eventId = generateId();
  ...
  it("PUT /events/:id/instances/:start detaches a single instance", async () => {
    ...
    const res = await request(app.getHttpServer())
      .put(`/calendar-events/${eventId}/instances/${instanceStart}`)
